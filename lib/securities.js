  ...
  function Security(master, 代碼1, 策略, 百分比) {
    ...
    hists({
      ...
      return function(err, arr) {
        ...
        if (err) {
          console.error("securities.coffee >> 29: " + _this.代碼 + " 下載5分鐘線", err);
          return master.重載(_this.代碼);
        } else {
          if (arr == null) {
    ...
    hists({
      ...
      return function(err, arr) {
        ...
        len = 0;
        if (err != null) {
          console.error("securities.coffee >> 65: " + _this.代碼 + " 下載週線", err);
          master.重載(_this.代碼);
        ...
        return hists({
          ...
          var ref;
          if (err != null) {
            console.error("securities.coffee >> 81: " + _this.代碼 + " 下載日線 " + (len * 5) + (ref = _this.代碼, indexOf.call(master.codes, ref) >= 0), err);
            return master.重載(_this.代碼);
          } else if ((arr != null ? arr.length : void 0) > 0) {
            pool = new 池();
...
Securities = (function() {
  ...
  Securities.prototype.重載 = function(code) {
    return this.品種[code] = new Security(this, code, this.策略, 0.618);
  };
