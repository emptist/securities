...
Security = (function() {
  function Security(master, 代碼, 證券名稱1, 策略, 百分比) {
    this.代碼 = 代碼;
    this.證券名稱 = 證券名稱1;
    this.策略 = 策略;
...
Securities = (function() {
  ...
  *
  ...
  Securities.prototype.生成載入 = function(tick, 回執) {
    var symbol, 證券, 證券名稱;
    symbol = tick.symbol;
    證券名稱 = tick.證券名稱;
    證券 = new Security(this, symbol, 證券名稱, this.策略);
      ...
      if (indexOf.call(this.symbols, symbol) >= 0) {
        if (this.品種[symbol] != null) {
          ...
          *
          ...
          results.push(this.生成載入(tick, 回執));
        }
      } else {
