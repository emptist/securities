    ...
    for (code in jso) {
      ...
      if (this.清潔) {
        if ((ref = this.品種[symbol]) != null ? ref.就緒 : void 0) {
          console.info("securities >> " + symbol + " position? ", this.position);
          if (this.position.length > 0) {
            if (indexOf.call(this.position, symbol) < 0) {
              console.info("securities >> " + symbol + " 可觀察? ", this.品種[symbol].可觀察);
              if (!this.品種[symbol].可觀察) {
                this.品種[symbol].clearIntervals();
                delete this.品種[symbol];
                this.symbols.splice(this.symbols.indexOf(symbol));
                results.push(console.info("securities >> " + symbol + " 不可買, 已去除"));
              } else {
                results.push(void 0);
              }
            } else {
