// Generated by CoffeeScript 1.10.0

/* 智能化的投資品種,
  根據行情結構,應對最新行情作出相應操作
  將逐步演化完善
  以下代碼等名目,僅僅是用於對接現有的Python接口,將來系統中都可以統一換成中文
 */
var Securities, Security,
  indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

Security = (function() {
  function Security(master, 代碼, 策略, 百分比) {
    this.百分比 = 百分比 != null ? 百分比 : 0.618;

    /* 經過如下處理,@對策 function中的this即此證券品種
     */
    this.策略 = 策略;
    this.對策 = this.策略.對策;
    this.代碼 = 代碼;
    this.策略.定制(master, this, (function(_this) {
      return function(err, done) {
        if (err == null) {
          return console.log("生成", _this.代碼);
        }
      };
    })(this));
  }

  Security.prototype.clearIntervals = function() {
    var each, i, len, ref, results;
    ref = this.intervals;
    results = [];
    for (i = 0, len = ref.length; i < len; i++) {
      each = ref[i];
      results.push(clearInterval(each));
    }
    return results;
  };

  Security.prototype.toString = function() {
    return "a Security 代碼: " + this.代碼;
  };


  /*合適: (回應)->
    @代碼? and 回應 this
  止損: (最新, 回應)->
    @對策(最新, 回應)
  #為何不行? 應對: @止損
   */

  Security.prototype.應對 = function(最新, 回應) {
    return this.對策(最新, 回應);
  };

  return Security;

})();


/* 目標證券群
  此處控制系統中同一個策略每一品種僅需一個object

  根據外部所提供行情,以及預定策略,實時決策操作
 */


/*
  @symbols: ['135333','xxdge']
 */

Securities = (function() {
  function Securities(symbols1, 策略1) {
    var i, len, ref, symbol;
    this.symbols = symbols1;
    this.策略 = 策略1;
    this.清潔 = false;
    this.position = [];
    this.品種 = {};
    ref = this.symbols;
    for (i = 0, len = ref.length; i < len; i++) {
      symbol = ref[i];
      this.品種[symbol] = new Security(this, symbol, this.策略);
    }
  }

  Securities.prototype.重載 = function(symbol) {
    return this.品種[symbol] = new Security(this, symbol, this.策略);
  };


  /* 從券商賬戶讀取的持倉品種,必須繼續跟蹤,以便止盈止損
   */

  Securities.prototype.持倉品種 = function(symbols) {
    var i, len, symbol;
    if (symbols != null) {
      for (i = 0, len = symbols.length; i < len; i++) {
        symbol = symbols[i];
        if (indexOf.call(this.position, symbol) < 0) {
          this.position.push(symbol);
        }
      }
    }
    return this.更新品種(symbols);
  };

  Securities.prototype.更新品種 = function(symbols) {
    var i, len, results, symbol;
    if (symbols != null) {
      results = [];
      for (i = 0, len = symbols.length; i < len; i++) {
        symbol = symbols[i];
        if (indexOf.call(this.symbols, symbol) < 0) {

          /* 須檢測 symbol 是否正常?
          #if symbol.length is 6
           */
          this.symbols.push(symbol);
          results.push(this.重載(symbol));
        } else {
          results.push(void 0);
        }
      }
      return results;
    }
  };

  Securities.prototype.應對 = function(jso, 回應) {
    var code, ref, results, symbol, tick;
    results = [];
    for (code in jso) {
      tick = jso[code];
      symbol = tick.代碼;

      /* 剔除不需要繼續跟蹤的品種
       */
      if (this.清潔 && (this.position.length > 0) && !(indexOf.call(this.position, symbol) >= 0) && ((ref = this.品種[symbol]) != null ? ref.不可買 : void 0)) {
        this.品種[symbol].clearIntervals();
        delete this.品種[symbol];
        results.push(this.symbols.splice(this.symbols.indexOf(symbol)));
      } else {
        if (indexOf.call(this.symbols, symbol) < 0) {
          console.log('securities 應對 新出現 tick.代碼:', symbol);
          if (symbol !== 'sz') {
            this.symbols.push(symbol);
            this.品種[symbol] = new Security(this, symbol, this.策略);
          }
        }
        if (symbol !== 'sz') {
          results.push(this.品種[symbol].應對(tick, 回應));
        } else {
          results.push(void 0);
        }
      }
    }
    return results;
  };

  Securities.prototype.clearIntervals = function() {
    var each, i, len, ref, results;
    console.log('securities>> clearIntervals');
    ref = this.symbols;
    results = [];
    for (i = 0, len = ref.length; i < len; i++) {
      each = ref[i];
      results.push(this.品種[each].clearIntervals());
    }
    return results;
  };

  return Securities;

})();

module.exports = Securities;

//# sourceMappingURL=securities.js.map
