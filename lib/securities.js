// Generated by CoffeeScript 1.10.0
var Securities, Security, hists, 分鐘, 小時, 池, 秒,
  indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

hists = require('sedata').hists;

池 = require('seyy').池;


/* 智能化的投資品種,
  根據行情結構,應對最新行情作出相應操作
  將逐步演化完善
  以下代碼等名目,僅僅是用於對接現有的Python接口,將來系統中都可以統一換成中文
 */

秒 = 1000;

分鐘 = 60 * 秒;

小時 = 60 * 分鐘;

Security = (function() {
  function Security(master, 代碼1, 策略, 百分比) {
    var 代碼;
    this.代碼 = 代碼1;
    this.策略 = 策略;
    this.百分比 = 百分比 != null ? 百分比 : 0.0618;
    if (this.代碼.length < 6) {
      console.error(this.代碼 + " 代碼不對");
    }

    /* 經過如下處理,@對策 function中的this即此證券品種
     */
    this.對策 = this.策略.對策;

    /* 在此取得行情,準備好天地線和頂底指標,
      也可在 observer中做.好處是此處不用再改寫.
      每隔 n分鐘更新一次 n分鐘數據
     */
    代碼 = this.代碼;
    hists({
      symbol: 代碼,
      type: 'm05'
    }, (function(_this) {
      return function(err, arr) {
        var pool, updateM05, 五分鐘線池;
        if (err) {
          console.error("securities.coffee >>: " + _this.代碼 + " 下載5分鐘線", err);
          return master.重載(_this.代碼);
        } else {
          if ((arr != null ? arr.length : void 0) > 0) {
            pool = new 池();
            _this.五分鐘線池 = pool.序列(arr);
            五分鐘線池 = _this.五分鐘線池;
            updateM05 = function() {
              return hists({
                symbol: 代碼,
                type: 'm05',
                len: 1
              }, function(err, arr) {
                if (err) {
                  return console.error("securities.coffee >>: " + this.代碼 + " 更新5分鐘線", err);
                } else if (arr[0].day !== 五分鐘線池.燭線.slice(-1)[0].day) {
                  return 五分鐘線池.新增(arr[0]);
                }
              });
            };
            return _this.iM05 = setInterval(updateM05, 5 * 分鐘);
          } else {
            console.error("securities.coffee >>: " + _this.代碼 + " 下載5分鐘線", err);
            return master.重載(_this.代碼);
          }
        }
      };
    })(this));
    hists({
      symbol: this.代碼,
      type: 'week',
      len: 1000
    }, (function(_this) {
      return function(err, arr) {
        var len, pool;
        len = 0;
        if (err != null) {
          console.error("securities.coffee >>: " + _this.代碼 + " 下載週線", err);
          master.重載(_this.代碼);
        } else if ((arr != null ? arr.length : void 0) > 0) {
          pool = new 池();
          _this.週線池 = pool.序列(arr);

          /* 用週線確定所需的行情片段再獲取日線,以免數據太大
          每隔24小時,在閉市期間更新一次日線數據
          排查發現個別品種下載數據會出錯
           */
          len = _this.週線池.求從魚長();
        } else {
          console.error("securities.coffee >>: " + _this.代碼 + " 下載週線", err);
          master.重載(_this.代碼);
        }
        return hists({
          symbol: _this.代碼,
          type: 'day',
          len: len * 5
        }, function(err, arr) {
          var ref;
          if (err != null) {
            console.error("securities.coffee >>: " + _this.代碼 + " 下載日線 " + (len * 5) + (ref = _this.代碼, indexOf.call(master.codes, ref) >= 0), err);
            return master.重載(_this.代碼);
          } else if ((arr != null ? arr.length : void 0) > 0) {
            pool = new 池();
            return _this.日線池 = pool.序列(arr);
          }
        });
      };
    })(this));
  }

  Security.prototype.clearIntervals = function() {
    var each, i, len1, ref, results;
    ref = [this.iM05, this.iDay, this.iWeek];
    results = [];
    for (i = 0, len1 = ref.length; i < len1; i++) {
      each = ref[i];
      results.push(clearInterval(each));
    }
    return results;
  };

  Security.prototype.toString = function() {
    return "a Security 代碼: " + this.代碼;
  };


  /*合適: (回應)->
    @代碼? and 回應 this
  止損: (最新, 回應)->
    @對策(最新, 回應)
  #為何不行? 應對: @止損
   */

  Security.prototype.應對 = function(最新, 回應) {
    return this.對策(最新, 回應);
  };

  return Security;

})();


/* 目標證券群
  此處控制系統中同一個策略每一品種僅需一個object

  根據外部所提供行情,以及預定策略,實時決策操作
 */


/*
  @codes: ['135333','xxdge']
 */

Securities = (function() {
  function Securities(codes1, 策略) {
    var code, i, len1, ref;
    this.codes = codes1;
    this.策略 = 策略;
    this.品種 = {};
    ref = this.codes;
    for (i = 0, len1 = ref.length; i < len1; i++) {
      code = ref[i];
      this.品種[code] = new Security(this, code, this.策略, 0.618);
    }
  }

  Securities.prototype.重載 = function(code) {
    return this.品種[code] = new Security(this, code, this.策略, 0.618);
  };

  Securities.prototype.更新品種 = function(codes) {
    var code, i, len1, results;
    if (codes != null) {
      results = [];
      for (i = 0, len1 = codes.length; i < len1; i++) {
        code = codes[i];
        if (indexOf.call(this.codes, code) < 0) {

          /* 須檢測 code 是否正常?
          #if code.length is 6
           */
          this.codes.push(code);
          results.push(this.重載(code));
        } else {
          results.push(void 0);
        }
      }
      return results;
    }
  };

  Securities.prototype.應對 = function(jso, 回應) {
    var code, k, results, tick;
    results = [];
    for (k in jso) {
      tick = jso[k];
      code = tick.代碼;
      if (indexOf.call(this.codes, code) < 0) {
        console.log('securities 應對 新出現 tick.代碼:', code);
        if (code !== 'sz') {
          this.codes.push(code);
          this.品種[code] = new Security(this, code, this.策略, 0.618);
        }
      }
      if (code !== 'sz') {
        results.push(this.品種[code].應對(tick, 回應));
      } else {
        results.push(void 0);
      }
    }
    return results;
  };

  Securities.prototype.clearIntervals = function() {
    var each, i, len1, ref, results;
    console.log('securities>> clearIntervals');
    ref = this.codes;
    results = [];
    for (i = 0, len1 = ref.length; i < len1; i++) {
      each = ref[i];
      results.push(this.品種[each].clearIntervals());
    }
    return results;
  };

  return Securities;

})();

module.exports = Securities;

//# sourceMappingURL=securities.js.map
