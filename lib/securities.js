...
Security = (function() {
  function Security(master, 代碼1, 策略, 百分比) {
    var 代碼;
    this.代碼 = 代碼1;
    this.策略 = 策略;
    ...
    hists({
      ...
      return function(err, arr) {
        var i, pool, updateM05, 五分鐘線池;
        if (err) {
          i = master.codes.indexOf(_this.代碼);
          master.codes.splice(i, 1);
          return console.error("securities.coffee >> 29: " + _this.代碼 + " 下載5分鐘線", err);
        } else {
          if (arr == null) {
          ...
          if ((arr != null ? arr.length : void 0) > 0) {
            ...
            updateM05 = function() {
              return hists({
                ...
                if (err) {
                  return console.error("securities.coffee >> 47: " + this.代碼 + " 更新5分鐘線", err);
                } else if (arr[0].day !== 五分鐘線池.燭線.slice(-1)[0].day) {
    ...
    hists({
      ...
      return function(err, arr) {
        ...
        if (err != null) {
          i = master.codes.indexOf(_this.代碼);
          master.codes.splice(i, 1);
          console.error("securities.coffee >> 66: " + _this.代碼 + " 下載週線", err);
        } else if ((arr != null ? arr.length : void 0) > 0) {
          pool = new 池();
        ...
        return hists({
          ...
          if (err != null) {
            i = master.codes.indexOf(_this.代碼);
            master.codes.splice(i, 1);
            return console.error("securities.coffee >> 81: " + _this.代碼 + " 下載日線 " + (len * 5) + (ref = _this.代碼, indexOf.call(master.codes, ref) >= 0), err);
          } else if ((arr != null ? arr.length : void 0) > 0) {
            pool = new 池();
...
Securities = (function() {
  function Securities(codes1, 策略) {
    ...
    for (j = 0, len1 = ref.length; j < len1; j++) {
      code = ref[j];
      this.品種[code] = new Security(this, code, this.策略, 0.618);
    }
  }

  Securities.prototype.更新品種 = function(codes) {
    var code, j, len1, results;
    if (codes != null) {
      results = [];
      for (j = 0, len1 = codes.length; j < len1; j++) {
        ...
        if (indexOf.call(this.codes, code) < 0) {
          ...
          results.push(this.品種[code] = new Security(this, code, this.策略, 0.618));
        } else {
          results.push(void 0);
        }
      }
